...
function [dphia,dphib,dphic] = calcPhi_Om0_moelans(phia,phib,phic,params)
    zero = 1e-15;
    nx = params.nx;
    ny = params.ny;
    dx = params.dx;
    ...
    dphia = - kzero(i,j).*laplace_a...
            ...
            + 1.5.*params.O.*phia(i,j).*phib(i,j).^2 + 1.5.*params.O.*phia(i,j).*phic(i,j).^2 + 0.5.*params.O.*(phia(i,j).^3 - phia(i,j));
    dphib = - kzero(i,j).*laplace_b...
            ...
            + 1.5.*params.O.*phib(i,j).*phia(i,j).^2 + 1.5.*params.O.*phib(i,j).*phic(i,j).^2 + 0.5.*params.O.*(phib(i,j).^3 - phib(i,j));
    dphic = - kzero(i,j).*laplace_c...
            ...
            + 1.5.*params.O.*phic(i,j).*phia(i,j).^2 + 1.5.*params.O.*phic(i,j).*phib(i,j).^2 + 0.5.*params.O.*(phic(i,j).^3 - phic(i,j));
