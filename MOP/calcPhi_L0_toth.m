...
function [dphia,dphib,dphic] = calcPhi_L0_toth(phia,phib,phic,params)
    zero = 1e-15;
    nx = params.nx;
    ny = params.ny;
    dx = params.dx;
    ...
    dphia = - kzero(i,j).*laplace_a...
            ...
            + Oab.*phia(i,j).*phib(i,j).^2 + Oac.*phia(i,j).*phic(i,j).^2 + Ozero(i,j).*(phia(i,j).^3 - phia(i,j).^2)...
            + 2.*phia(i,j).*((Oab-Ozero(i,j)).*phib(i,j).^2+(Oac-Ozero(i,j)).*phic(i,j).^2)./sumSquares(i,j).*((phia(i,j).^4 + phib(i,j).^4 + phic(i,j).^4)./4 - (phia(i,j).^3 +phib(i,j).^3 +phic(i,j).^3)./3 + 1/12);
    dphib = - kzero(i,j).*laplace_b...
            ...
            + Oab.*phib(i,j).*phia(i,j).^2 + Obc.*phib(i,j).*phic(i,j).^2 + Ozero(i,j).*(phib(i,j).^3 - phib(i,j).^2)...
            + 2.*phib(i,j).*((Oab-Ozero(i,j)).*phia(i,j).^2+(Obc-Ozero(i,j)).*phic(i,j).^2)./sumSquares(i,j).*((phia(i,j).^4 + phib(i,j).^4 + phic(i,j).^4)./4 - (phia(i,j).^3 +phib(i,j).^3 +phic(i,j).^3)./3 + 1/12);
    dphic = - kzero(i,j).*laplace_c...
            ...
            + Oac.*phic(i,j).*phia(i,j).^2 + Obc.*phic(i,j).*phib(i,j).^2 + Ozero(i,j).*(phic(i,j).^3 - phic(i,j).^2)...
            + 2.*phic(i,j).*((Oac-Ozero(i,j)).*phia(i,j).^2+(Obc-Ozero(i,j)).*phib(i,j).^2)./sumSquares(i,j).*((phia(i,j).^4 + phib(i,j).^4 + phic(i,j).^4)./4 - (phia(i,j).^3 +phib(i,j).^3 +phic(i,j).^3)./3 + 1/12);
end