...
folder = ['..\..\Simulations\Benchmark-2\MPF-grad1-obstacle\'];
grid   = ['100x200'];
...
for gamma=gbc
    ...
    for k=1:frames
        ...
        switch benchmark
            case 'static'
                [Ftot(k),TP_x(k),TP_y(k),ghost(k,:)] = calc_metrics_static(gradient,potential,phia,phib,phic,fig_TP,params);
            case 'steadyState'
                [Ftot(k),TP_x(k),TP_y(k),max_GB(k),h_GB(k),L2(k),ghost(k,:)] = calc_metrics_steadyState(gradient,potential,phia,phib,phic,fig_TP,params);
        end
